...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    std::vector<JSAMPLE> jsample_buf(row_stride);
    ...
    for (;image_begin < image_end; ++image_begin) {
        jsample_buf[index] = static_cast<JSAMPLE>(image_begin->r);
        jsample_buf[index + 1] = static_cast<JSAMPLE>(image_begin->g);
        jsample_buf[index + 2] = static_cast<JSAMPLE>(image_begin->b);

        index += 3;
        } 
        row_pointer[0] = jsample_buf.data();
